...
delete:
	if [ -n "$$(docker ps -aqf ancestor=gelignite/fda_node_online-cinema)" ] && [ -n "$$(docker ps -aqf ancestor=gelignite/fda_next_online-cinema)" ] && [ -n "$$(docker ps -aqf ancestor=nginx)" ] && [ -n "$$(docker ps -aqf ancestor=certbot/certbot)" ]; then \
		...
		docker stop $$(docker ps -aqf ancestor=certbot/certbot) && docker rm $$(docker ps -aqf ancestor=certbot/certbot); \
	fi; \
