...
export const getStaticPaths: GetStaticPaths = async () => {
	try {
		const { data: actors } = await ActorServiceBuild.getAll()
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
	try {
		const { data: actor } = await ActorServiceBuild.getBySlug(
			params?.slug as string
		)
		const { data: movies } = await MovieServiceBuild.getByActor(actor._id)
